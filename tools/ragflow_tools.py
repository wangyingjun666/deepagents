...
def _get_ragflow_client():
    """惰性创建 RAGFlow 客户端；配置缺失或创建失败时返回 None。"""
    ...
    if not api_key or not base_url:
        print("[RAGFlow] 未配置 RAGFLOW_API_URL / RAGFLOW_API_KEY，知识库助手不可用。")
        return None
...
def get_assistant_list() -> str:
    ...
    if ragflow_client is None:
        return "RAGFlow知识库不可用：缺少有效的 RAGFLOW_API_URL / RAGFLOW_API_KEY 配置。"
